"""
list_tools/tool.py — Tool for the LLM to discover available tools and their schemas.
...
Category info is dynamically read from TOOL_CATEGORIES at runtime,
so it always reflects the current state of metadata.md tags.
...
# ============================================================
# Category map (lazy import — avoids circular dep)
# ============================================================
def _get_categories():
...
class ListToolsInput(BaseModel):
    ...
    category: str = Field(
        default="all",
        description=(
            "Tool category filter. Use 'all' to see every registered tool "
            "with its current categories. "
            "Specific category keys are discovered by calling category='all' first."
        ),
    )
    tool_name: Optional[str] = Field(
        default=None,
        description=(
            "Exact tool name to get detailed schema (e.g., 'oligowalk', 'primer3'). "
